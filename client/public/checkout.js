...
async function displayOrderSummary(orderSummary) {
    ...
    console.log('Mostrando resumen:', orderSummary);
    ...
    // Calcular totales
    const discountPercent = parseFloat(orderSummary.discount?.percent || currentDiscount.percent || 0);
    const discountAmount = subtotal * (discountPercent / 100);
    const taxableAmount = subtotal - discountAmount;
    const tax = taxableAmount * 0.21;
    const total = taxableAmount + tax;

    console.log('Totales:', { subtotal, discountAmount, tax, total });
    ...
    orderData = {
        ...
        tax: tax,
        total: total,
        discountCode: orderSummary.discount?.code || currentDiscount.code
    };
}
    ...
    try {
        ...
        if (result.success) {
            ...
            localStorage.setItem('lastOrder', JSON.stringify({
                orderNumber: result.data.numero_pedido || Math.floor(Math.random() * 100000),
                ...orderData,
                customer: orderPayload.customer,
                shipping: orderPayload.shipping,
                payment: { method: orderPayload.payment.method }
            }));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.payment-method').forEach(method => {
        method.addEventListener('click', function(e) {
            e.preventDefault();
            console.log('Método seleccionado:', this.dataset.method);
            ...
            console.log('Valor guardado:', document.getElementById('paymentMethod').value);
